...
namespace Ticsa.DAL.DP {
    public abstract class StdDP<T> where T : StdEntity, new() {
        ...
        public async Task<T> GetById(int id) =>
            (await GetConnection().QueryAsync<T>($"{_SpGetByIdLabel}", BuildGetByIdParam(id), commandType: System.Data.CommandType.StoredProcedure)).FirstOrDefault();
        ...
        public async Task<T> Update(T entity) =>
            (await GetConnection().ExecuteAsync($"{_SpUpdateLabel}", BuildUpdateParam(entity), commandType: System.Data.CommandType.StoredProcedure)) != 0 ? entity : throw new Exception($"Fail to update entity typeof({typeof(T).Name})");
        public async Task<bool> Delete(int id) =>
            (await GetConnection().ExecuteAsync($"{_SpDeleteLabel}", BuildDeleteParam(id), commandType: System.Data.CommandType.StoredProcedure)) != 0 ? true : throw new Exception($"Fail to delete entity typeof({typeof(T).Name})");

        protected abstract object BuildGetByIdParam(int id);
        protected abstract object BuildAddParam(T entity);
        protected abstract object BuildUpdateParam(T entity);
        protected abstract object BuildDeleteParam(int id);
    }
}
