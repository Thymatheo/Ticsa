...
namespace Ticsa.DAL.DP {
    public abstract class StdDP<T> where T : StdEntity, new() {
        ...
        public async Task<T> GetById(int id) =>
            (await GetConnection().QueryAsync<T>($"{_SpGetByIdLabel}", id, commandType: System.Data.CommandType.StoredProcedure)).FirstOrDefault();
        ...
        public async Task<bool> Delete(int id) =>
            (await GetConnection().ExecuteAsync($"{_SpDeleteLabel}", id, commandType: System.Data.CommandType.StoredProcedure)) != 0 ? true : throw new Exception($"Fail to delete entity typeof({typeof(T).Name})");
